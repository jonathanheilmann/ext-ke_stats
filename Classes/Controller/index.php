...
define('TYPO3_MOD_PATH', '../typo3conf/ext/ke_stats/Classes/Controller/');
$BACK_PATH='../../../../../typo3/';
...
class Module1Controller extends \TYPO3\CMS\Backend\Module\BaseScriptClass {
	...
	public function __construct()
	{
		...
		$this->getLanguageService()->includeLLFile('EXT:ke_stats/mod1/locallang.xml');
	...
	function main()	{
		...
		if (($this->id && $access))	{
			...
			$this->doc->backPath = $BACK_PATH;
				...
				// initialize tab menu
			$this->tabmenu = GeneralUtility::makeInstance('Kennziffer\\KeStats\\Utility\\BackendmenuUtility');
			...
			//$this->content .= $this->doc->section('',$this->doc->funcMenu($headerSection,TYPO3\CMS\Backen\Utility\BackendUtility::getFuncMenu($this->id,'SET[function]',$this->MOD_SETTINGS['function'],$this->MOD_MENU['function'])));
			...
			*/
			...
			if ($this->tabmenu->getSelectedValue('type') != 'overview' && $this->tabmenu->getSelectedValue('type') != 'csvdownload' && !$this->csvOutput) {
				...
				// Current page title
			if ($this->id) {
				$row = TYPO3\CMS\Backend\Utility\BackendUtility::getRecord('pages',$this->id);
				$pagetitle = TYPO3\CMS\Backend\Utility\BackendUtility::getRecordTitle('pages',$row,1);
...
// Make instance:
$GLOBALS['SOBE'] = GeneralUtility::makeInstance('Kennziffer\\KeStats\\Controller\\Module1Controller');
$GLOBALS['SOBE']->init();
